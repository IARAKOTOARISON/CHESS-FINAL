 ...
 */
public class ImageEchiquierGenerator {
    public static final int CELL_SIZE = 64; // pixels par case
    public static final int BOARD_SIZE = 8;
    public static final int MARGIN = 24;    // marge autour du plateau
    ...
    public static void generate(String outputPath) throws IOException {
        // Historique: génère une image de plateau initial pour des usages externes.
        BufferedImage img = renderPlateau(new Plateau());
    ...
    /**
     * Rend une image du plateau courant (cases + pièces du plateau fourni).
     */
    public static BufferedImage renderPlateau(Plateau plateau) {
        ...
        // Dessiner les pièces du plateau fourni (ignorant les pièces nulles ou "mortes")
        for (int y = 0; y < BOARD_SIZE; y++) {
            for (int x = 0; x < BOARD_SIZE; x++) {
    ...
    /**
     * Rend une image du plateau et dessine une trajectoire par-dessus.
     */
    public static BufferedImage renderPlateauWithTrajectory(Plateau plateau, java.util.List<CalculateurTrajectoire.Point> traj) {
        BufferedImage base = renderPlateau(plateau);
        if (traj == null || traj.isEmpty()) return base;
        ...
        for (int i = 0; i < traj.size() - 1; i++) {
            var p1 = traj.get(i);
            var p2 = traj.get(i + 1);
            int x1 = MARGIN + (int)(p1.x * CELL_SIZE + CELL_SIZE / 2.0);
            int y1 = MARGIN + (int)((7 - p1.y) * CELL_SIZE + CELL_SIZE / 2.0);
            int x2 = MARGIN + (int)(p2.x * CELL_SIZE + CELL_SIZE / 2.0);
            int y2 = MARGIN + (int)((7 - p2.y) * CELL_SIZE + CELL_SIZE / 2.0);
